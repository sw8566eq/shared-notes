// Package webview provides Go bindings for the native webview library
// (a thin C/C++ wrapper over each OS's own web-rendering component:
// WebKitGTK on Linux).
//
// This is a trimmed, patched vendored copy of
// github.com/webview/webview_go (commit fb6b17d, MIT license — see
// LICENSE in this directory), kept in-tree instead of pulled in as an
// external dependency for two reasons:
//
//  1. Upstream hardcodes the Linux build against pkg-config module
//     "webkit2gtk-4.0". Debian 13 (trixie) and other current distros
//     only ship "webkit2gtk-4.1" — the 4.0 dev package is gone, so
//     upstream fails to build there out of the box (tracked upstream,
//     unresolved: github.com/webview/webview_go issue #50). This copy
//     targets 4.1 directly instead of depending on a pkg-config shim
//     or an unmaintained fork.
//  2. linkshr's desktop client only needs Linux, so the Windows
...
#cgo CXXFLAGS: -I${SRCDIR}/include -DWEBVIEW_STATIC

#cgo CXXFLAGS: -DWEBVIEW_GTK -std=c++11
#cgo LDFLAGS: -ldl
